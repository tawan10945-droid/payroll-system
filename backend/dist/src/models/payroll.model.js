...
class Payroll {
    ...
    static async calculate(employee_id, start_date, end_date) {
        ...
        // Get bonuses
        const [bonusRows] = await pool.query(`SELECT COALESCE(SUM(amount), 0) as total_bonus 
             FROM bonuses 
             WHERE employee_id = ? AND bonus_date BETWEEN ? AND ?`, [employee_id, start_date, end_date]);
        ...
        const [result] = await pool.query(`INSERT INTO payroll 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`, [employee_id, start_date, end_date, basic_salary, overtime_pay, bonus_amount, gross_pay, tax_deduction, other_deductions, net_pay]);
    ...
    static async markAsPaid(id) {
        const [result] = await pool.query(`UPDATE payroll SET status = 'paid', payment_date = CURRENT_DATE, updated_at = CURRENT_TIMESTAMP WHERE payroll_id = ?`, [id]);
